...
optimizer = torch.optim.Adam([v], lr=25)
...
for epoch in range(num_epochs):
    ...
    for i, (s_d_i, s_a_i, x_s_i, x_r_i) in enumerate(train_loader):
        ...
        loss = (
            loss_fn(out.to(device),
                    s_d_i.to(device)) + alpha_tv * (ATVLoss(v))/(model_dim[0]*model_dim[1])
        )
    ...
    if epoch % 20 == 0:
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            v.detach().cpu().numpy().tofile(str(epoch) + 'adam_marmousi2_inv.bin')
...
v_save.tofile('fin_adam_marmousi2_inv.bin')
