...
def get_mgrid(sidelen_z, sidelen_x, dim=2):
    '''Generates a flattened grid of (x,y,...) coordinates in a range of -1 to 1.
    sidelen: int
    dim: int'''
    tensors = tuple([torch.linspace(-1, 1, steps=sidelen_z), torch.linspace(-1, 1, steps=sidelen_x)])
    mgrid = torch.stack(torch.meshgrid(*tensors), dim=-1)
    mgrid = mgrid.reshape(-1, dim)
    return mgrid

# [1, 114*200, 2] input size
net_input_grid = get_mgrid(220, 128, dim=2)
...
net_input = torch.zeros(1,64,11,5)
observed_data_flap_nor = (((net_input_grid.reshape(-1))[0:28160].reshape(1,-1,11,5))[:,0:512,:,:])
for index in range(64):
    net_input[:,index,:,:] = torch.mean(observed_data_flap_nor[:,index::64,:,:], 1)

net_input = (net_input - net_input.mean())/net_input.std()
