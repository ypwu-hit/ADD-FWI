...
net_input = torch.randn(1,3,352,144)
...
in_channels = 3
output_channels = 1
input_height = model_dim[0]
input_width = model_dim[1]

net = Network(channel_init, embedding_size, in_channels, output_channels, input_height, input_width)

print(net)

torch.save(net.state_dict(),'nnfwi_ae_init.pkl')
...
for epoch in range(num_epochs):
    ...
    if epoch % 20 == 0:
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            v.detach().cpu().numpy().tofile(str(epoch) + '_marmousi2_inv.bin')
            ...
            plt.colorbar()
            plt.tight_layout()
            plt.savefig(str(epoch) + 'ae_wdp.png')
            plt.close()

            outputs.detach().cpu().numpy().tofile(str(epoch) + '_ae_wdp_marmousi2_inv.bin')
