...
torch.save(net.state_dict(),'nnfwi_unet_init.pkl')
...
for epoch in range(num_epochs):
    ...
    if epoch % 20 == 0:
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            plt.colorbar()
            plt.tight_layout()
            plt.savefig(str(epoch) + 'unet_wdp.png')
            plt.close()

            outputs.detach().cpu().numpy().tofile(str(epoch) + '_unet_wdp_marmousi2_inv.bin')
            torch.save(net.state_dict(),'nnfwi3_unet.pkl')
