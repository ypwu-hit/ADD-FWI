...
v_mig = torch.where(v_true == 1500, v_true, 2000)
...
net_input = torch.randn(1,64,7,4)
...
for epoch in range(num_epochs):
    ...
    for i, (s_d_i, s_a_i, x_s_i, x_r_i) in enumerate(train_loader):
        ...
        # Forward prediction
        outputs = net(net_input.to(device)).mean(dim=[0,1], keepdim=False)
        v_temp = v_mig.clone()
        v_temp[:,16:] = outputs
    ...
    if epoch % 20 == 0:
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            observed_data_fd = scalar(v_temp, dx, dt,
            ...
            plt.figure()
            plt.imshow(v_temp.detach().cpu().numpy().T, aspect='auto', cmap='jet', vmax=4700, vmin=1500)
            plt.xlabel("X")
            plt.ylabel("Z")
            plt.title("Velocity")
            plt.colorbar()
            ...
            v_temp.detach().cpu().numpy().tofile(str(epoch) + '_unet_marmousi2_inv.bin')
