...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.permute(0, 2, 1).view(shape[0], shape[1], shape[2], shape[3]).contiguous()
...
class VanillaConvUpDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_attention_1 = nn.Sequential(
                MultiHeadAttention(dim=in_channels, n_head=8),
        ...
        self.patch_attention_2 = nn.Sequential(
                MultiHeadAttention(dim=hidden_dims[0], n_head=8),
                nn.BatchNorm2d(hidden_dims[0]),
                nn.ReLU(inplace=True)
                )
