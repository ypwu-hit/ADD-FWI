...
class MultiHeadAttention_patch(nn.Module):
    def __init__(
        self, dim=11*5*64, n_head=8, h=11,w=5,c=64,p1=2,p2=2
    ):
        super().__init__()
        ...
        self.rerange1 = nn.Sequential(
            Rearrange("b c (p1 h) (p2 w) -> b (p1 p2) (h w c)", p1=p1, p2=p2, h=h, w=w),
        )

        self.rerange2 = nn.Sequential(
            Rearrange("b (p1 p2) (h w c) -> b c (p1 h) (p2 w)", p1=p1, p2=p2, c=c, h=h, w=w),        
        )
    
    def forward(self, x):
        
        # self.rerange(x) -> (b c h w) -> (b (p1 p2) (h w c))
...
class VanillaConvUpDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_attention_1 = nn.Sequential(
                MultiHeadAttention_patch(dim=11*5*64, n_head=8, h=11,w=5,c=64,p1=2,p2=2),
        ...
        self.patch_attention_2 = nn.Sequential(
                MultiHeadAttention_patch(dim=11*5*64, n_head=8, h=11,w=5,c=64,p1=4,p2=4),
                nn.BatchNorm2d(hidden_dims[0]),
                nn.ReLU(inplace=True)
                )
